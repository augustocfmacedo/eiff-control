...
export function paginar(buscarPagina: (p: { page_size: number; page?: number; next_cursor?: string }) => Promise<{ data?: Record<string, unknown>[]; page?: { next_cursor?: string | null }; next_cursor?: string | null; pagination?: { next_cursor?: string | null }; response_context?: { correlation_id?: string } }>, opts: { businessIds: string[]; escolhidos: Map<string, Record<string, unknown>>; max: number; maxPaginas?: number; extra?: Record<string, unknown>; cap?: number; orcamento?: number; custoPorRegistro?: number }): Promise<{ paginas: number; devolvidos: number; correlacoes: string[]; motivo: string; vistos: number }>;
...
export function linhasParaEnriquecer<T extends Record<string, unknown>>(linhas: T[], opts?: { force?: boolean; hoje?: string; cacheDias?: number }): T[];
...
export const POOL_JOB_LEVEL: string[];
export const POOL_JOB_DEPARTMENT: string[];
export function filtrosValidados(desejados: { job_level?: string[]; job_department?: string[] }, catalogo: { job_level?: string[]; job_department?: string[] } | null | undefined): { job_level: string[]; job_department: string[]; rejeitados: string[] };
export function filtrosPool(businessIds: string[], catalogo: { job_level?: string[]; job_department?: string[] } | null | undefined, somenteComEmail?: boolean): { filtros: Record<string, unknown>; rejeitados: string[] } | { erro: string; rejeitados: string[] };
export function cursorDe(r: unknown): string | null;
export function dentroDoCache(linha: Record<string, unknown>, hoje: string, cacheDias?: number): boolean;
