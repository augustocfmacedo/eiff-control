...
import { POOL_JOB_DEPARTMENT, POOL_JOB_LEVEL, TITULOS_ESPECIFICOS, classificarErro, filtrosPool, filtrosValidados, hashRequisicao, proximaPagina, tamanhoPaginaServidor, type CatalogoFiltros, type TipoOperacao } from '../../src/core/radar/vibeServidor';
...
export default async (req: Request): Promise<Response> => {
  ...
  const rpc = async (nome: string, args: Row): Promise<Row> => { const r = await fetch(`${url}/rest/v1/rpc/${nome}`, { method: 'POST', headers: cab, body: JSON.stringify(args) }); const d = await r.json().catch(() => ({})); if (!r.ok) throw new Error(`rpc ${nome}: ${mascarar(JSON.stringify(d).slice(0, 200))}`); return (d ?? {}) as Row; };
  ...
  if (!chave && !['orcamento', 'estado', 'cancelar'].includes(acao)) return json({ erro: 'nao_configurado', mensagem: 'VIBE_API_KEY não definida no painel do Netlify.' }, 501);
  ...
  try {
    ...
    if (acao === 'catalogo') {
      ...
      const up = await fetch(`${url}/rest/v1/radar_vibe_credit_policy?org_id=eq.${perfil.organization_id}`, { method: 'PATCH', headers: { ...cab, prefer: 'return=representation' }, body: JSON.stringify({ validated_filters: catalogo, validated_at: catalogo.validado_em, updated_by: u.id }) });
      const linhas = (await up.json().catch(() => [])) as unknown[];
      if (up.ok && !linhas.length) await fetch(`${url}/rest/v1/radar_vibe_credit_policy`, { method: 'POST', headers: cab, body: JSON.stringify({ org_id: perfil.organization_id, validated_filters: catalogo, validated_at: catalogo.validado_em, updated_by: u.id }) });
    ...
    if (acao === 'simular' || acao === 'reservar') {
      ...
      const r = await rpc('reserve_vibe_operation', { p_idempotency_key: key, p_request_hash: hashRequisicao(pr.params), p_operation_type: tipo, p_estimated_credits: pr.estimado, p_records_requested: pr.registros, p_credits_available: disp, p_paid_record_cap: pr.cap, p_dry_run: acao === 'simular' });
      return json({ ...r, disponiveis: disp, estimado: pr.estimado, registros: pr.registros, requestHash: hashRequisicao(pr.params) });
    }
    if (acao === 'cancelar') { const r = await rpc('update_vibe_operation', { p_id: corpo.operationId, p_status: 'CANCELLED', p_fields: { error_code: 'cancelada_pelo_usuario' } }); return json(r); }
    if (acao === 'concluir') {
      const disp = await creditos();
      const r = await rpc('update_vibe_operation', { p_id: corpo.operationId, p_status: 'SUCCEEDED', p_fields: { credits_after: disp } });
    ...
    const pol = await politica();
    const antes = await creditos();
    if (est.status === 'RESERVED') await rpc('update_vibe_operation', { p_id: opId, p_status: 'RUNNING', p_fields: { credits_before: antes } });
    const registrar = async (status: 'SUCCEEDED' | 'FAILED' | 'UNCERTAIN' | 'RUNNING', fields: Row) => rpc('update_vibe_operation', { p_id: opId, p_status: status, p_fields: fields });
    let enviado = false;
    try {
      if (tipo === 'match') {
