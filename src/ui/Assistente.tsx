  ...
  const enviar = async (q0?: string) => {
    ...
    try {
      if (ia !== 'indisponivel') {
        ...
        const r = await fetch('/api/assistente', { method: 'POST', headers: { 'content-type': 'application/json', ...(token ? { authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ mensagens: hist.slice(-12).map((m) => ({ papel: m.papel, texto: m.texto })), contexto: { papel: usuario.papel, nome: usuario.nome, tela: TITULOS['/' + tela.split('/')[1]] ?? tela, dataBase: ds.params.dataBase, resumo } }) });
