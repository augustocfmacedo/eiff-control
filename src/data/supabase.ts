...
import { montarDataset, type Refs } from './mapeamentoDataset';
...
export async function carregarRemoto(): Promise<{ ds: Dataset; usuario: Usuario }> {
  ...
  const montado = montarDataset({ perfis, orgs, companies, params, fatores, plano, contas, obras, lancs, liqs, trans, recs, dividas, aprovs, steps, coms, tasks, closes, audit, scopes, servicosRows, demandasRows, conclusoesRows, ordensRows, etapasRows, workers, timesheets, tsLines, tsOutputs, tsIncidents, medicoesRows, pedidosRows, pedidosItens, conjuntosRows, avancosRows, estacaoRows, romaneioRows, stockItems, stockMovs, trainingRows, fotoRows, funcRows, alocRows, insumosRows, compRows, compItens, estRows, estItens });
  refs = montado.refs;
  const ds = montado.ds;
  const usuario = ds.usuarios.find((u) => u.id === meu.id)!;
...
export const remotoPronto = () => !!supabase && !!refs;
...
export async function persistirRemoto(antes: Dataset, depois: Dataset, atorId: string): Promise<void> {
  ...
  // estoque: itens (update-senao-insert) e movimentos (imutaveis; estorno e um movimento novo)
  for (const i of mudou(antes.itensEstoque ?? [], depois.itensEstoque ?? [], 'id')) {
    const row = { code: i.codigo, description: i.descricao, family: i.familia, catalog_input_id: i.insumoId ? r.insumos.get(i.insumoId) ?? null : null, unit_weight: i.pesoUnitario ?? null, min_stock: i.estoqueMinimo, active: i.ativo, notes: i.observacoes || null };
    const data = await gravar('stock_item', { id: r.itensEstoque.get(i.id) }, row, { organization_id: r.orgId, created_by: atorId });
    if (data) r.itensEstoque.set(i.id, data.id);
  }
  for (const m of (depois.movimentosEstoque ?? []).filter((x) => !r.movEstoque.get(x.id))) {
