...
const SPECS: Spec<{ id: string }>[] = [
  ...
  { chave: 'importacaoLinhas', tabela: 'radar_import_row', ordem: 'row_number', imutavel: true, app: (x) => ({ id: x.id, jobId: x.job_id, numero: Number(x.row_number), dados: x.data ?? {}, status: x.status, entidadeId: s(x.entity_id), mensagem: s(x.message) }), db: (o: Row, ref) => ({ job_id: ref('importacoes', o.jobId), row_number: o.numero, data: o.dados, status: o.status, entity_id: ref('empresas', o.entidadeId) ?? ref('contatos', o.entidadeId), message: nn(o.mensagem) }) },
...
export async function carregarRadar(h: Pick<HelpersRadar, 'sel' | 'orgId'>): Promise<RadarDataset> {
  ...
  const [tiposResposta, configScore] = await Promise.all([h.sel('radar_response_type', 'code'), h.sel('radar_score_setting', 'key')]);
