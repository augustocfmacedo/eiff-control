// EIFF Inbox: dados de EXEMPLO para o bootstrap. Tudo ficticio: pessoas, empresas e numeros nao existem.
// Usado no modo local (seed) e, no modo remoto, so quando um Administrador pede "carregar exemplo" — nunca entra no banco.
// As datas sao relativas a `agoraIso` para que SLA, ordem de trabalho e caixas facam sentido em qualquer dia.
import { CONFIGURACAO_PADRAO, SETORES_PADRAO, type ContatoInbox, type InboxAction, type InboxDataset, type InboxJob, type InboxMessage, type InboxThread, type MembroSetor, type Setor, type Sugestao, type ThreadEvent } from './tipos';
...
export const MEMBROS_SEED: MembroSetor[] = [
  { usuarioId: 'u-fin', setorCodigo: 'FINANCEIRO', papel: 'gestor' },
  { usuarioId: 'u-contab', setorCodigo: 'FINANCEIRO', papel: 'atendente' },
  { usuarioId: 'u-obra', setorCodigo: 'OBRAS', papel: 'gestor' },
  { usuarioId: 'u-obra', setorCodigo: 'POS_VENDA', papel: 'gestor' },
  { usuarioId: 'u-eng', setorCodigo: 'OBRAS', papel: 'atendente' },
  { usuarioId: 'u-eng', setorCodigo: 'ENGENHARIA', papel: 'gestor' },
  { usuarioId: 'u-compras', setorCodigo: 'COMPRAS', papel: 'gestor' },
  { usuarioId: 'u-compras', setorCodigo: 'FORNECEDORES', papel: 'gestor' },
  { usuarioId: 'u-augusto', setorCodigo: 'COMERCIAL', papel: 'gestor' },
  { usuarioId: 'u-augusto', setorCodigo: 'DIRETORIA', papel: 'gestor' },
  { usuarioId: 'u-admin', setorCodigo: 'ADMINISTRATIVO', papel: 'gestor' },
  { usuarioId: 'u-admin', setorCodigo: 'SISTEMA', papel: 'gestor' },
];
...
export function seedInbox(agoraIso: string): InboxDataset {
  ...
  const msg = (m: Omit<InboxMessage, 'id' | 'anexos'> & { anexos?: InboxMessage['anexos'] }): InboxMessage => { const x: InboxMessage = { anexos: [], ...m, id: `MSG-${String(++nMsg).padStart(5, '0')}` }; mensagens.push(x); return x; };
  const evt = (e: Omit<ThreadEvent, 'id'>) => { eventos.push({ ...e, id: `EVT-${String(++nEvt).padStart(5, '0')}` }); };

  // T1 — fornecedor pergunta pela NF 583: classificada, roteada ao Financeiro, atribuida, ainda sem resposta (SLA no prazo)
  const t1: InboxThread = {
    ...
    setorCodigo: 'FINANCEIRO', responsavelId: 'u-fin', participantes: ['u-fin'], codigoObra: OBRA, labels: ['nota fiscal'],
    classificacao: { intencao: 'consultar_pagamento', assunto: 'Pagamento de nota fiscal', entidades: [{ tipo: 'nota_fiscal', valor: 'NF 583', mensagemId: 'MSG-00001' }, { tipo: 'obra', valor: OBRA }], setorRecomendado: 'FINANCEIRO', prioridadeRecomendada: 'Normal', nivelRecomendado: 'B', acaoSugerida: 'consultar previsão de pagamento no contas a pagar', confianca: 0.92, sinais: ['menciona número de NF', 'contato é fornecedor da obra', 'pergunta sobre liberação de pagamento'], evidencias: [{ mensagemId: 'MSG-00001', trecho: 'NF 583 já está liberada para pagamento' }], provedor: 'SEED', versao: 'exemplo-1', em: h(3) },
    resumoIa: 'Fornecedor de aço pergunta se a NF 583 (obra Smart Fit) já foi liberada para pagamento.',
  ...
  evt({ threadId: t1.id, tipo: 'ABERTA', em: h(3), ator: sistema, detalhe: 'conversa aberta por whatsapp (EXTERNAL)' });
  evt({ threadId: t1.id, tipo: 'CLASSIFICADA', em: h(3), ator: { tipo: 'ia', nome: 'Exemplo' }, detalhe: 'intenção consultar_pagamento · confiança 92%' });
  evt({ threadId: t1.id, tipo: 'ROTEADA', em: h(3), ator: sistema, detalhe: 'regra ROT-02: pagamento e cobrança → Financeiro; nível B: resposta com compromisso' });
  evt({ threadId: t1.id, tipo: 'ATRIBUIDA', em: h(3), ator: sistema, detalhe: 'responsável padrão do setor', depois: 'u-fin' });
  sugestoes.push({ id: 'SUG-00001', threadId: t1.id, tipo: 'resposta', texto: 'Olá, Paulo. Localizei a NF 583 no nosso contas a pagar da obra Smart Fit. Vou confirmar a data programada com o Financeiro e retorno por aqui ainda hoje.', estado: 'pendente', provedor: 'SEED', criadaEm: h(3) });

  // T2 — transportadora confirma descarga: Obras respondeu, aguardando o contato
  const t2: InboxThread = {
    ...
    setorCodigo: 'OBRAS', responsavelId: 'u-obra', participantes: ['u-obra'], codigoObra: OBRA, labels: ['logística'],
    ...
    resumoIa: 'Transportadora avisa que a estrutura chega amanhã e pede confirmação de descarga às 8h.',
  ...
  evt({ threadId: t2.id, tipo: 'ABERTA', em: h(5), ator: sistema, detalhe: 'conversa aberta por whatsapp (EXTERNAL)' });
  evt({ threadId: t2.id, tipo: 'ROTEADA', em: h(5), ator: sistema, detalhe: 'regra ROT-03: operação da obra → Obras / Operações · prioridade Alta' });
  evt({ threadId: t2.id, tipo: 'MENSAGEM', em: h(4), ator: usuario('u-obra'), detalhe: 'resposta registrada (canal não conectado: nada foi enviado)' });
  evt({ threadId: t2.id, tipo: 'STATUS', em: h(4), ator: usuario('u-obra'), detalhe: 'aguardando confirmação da transportadora', antes: 'EM_ATENDIMENTO', depois: 'AGUARDANDO_CONTATO' });
  ...
  const t3: InboxThread = {
    ...
    setorCodigo: 'ENGENHARIA', responsavelId: 'u-eng', participantes: ['u-eng'], codigoObra: OBRA, labels: ['projeto', 'orçamento'],
    ...
    resumoIa: 'Cliente pede revisão do projeto R02 com análise de impacto no orçamento até sexta.',
  ...
  evt({ threadId: t3.id, tipo: 'ABERTA', em: h(30), ator: sistema, detalhe: 'conversa aberta por whatsapp (EXTERNAL)' });
  evt({ threadId: t3.id, tipo: 'ROTEADA', em: h(30), ator: sistema, detalhe: 'regra ROT-06: projeto e engenharia → Engenharia' });
  evt({ threadId: t3.id, tipo: 'ACAO', em: h(27), ator: usuario('u-eng'), detalhe: 'ação proposta: revisar projeto R02 e medir impacto no orçamento (aguarda Diretoria)' });
  evt({ threadId: t3.id, tipo: 'STATUS', em: h(27), ator: usuario('u-eng'), detalhe: 'aguardando aprovação da Diretoria', antes: 'EM_ATENDIMENTO', depois: 'AGUARDANDO_APROVACAO' });
  acoes.push({ id: 'ACT-00001', threadId: t3.id, tipo: 'criar_job', titulo: 'Revisar projeto R02 e medir impacto no orçamento', descricao: 'Abrir revisão técnica da R02 (terças da cobertura) e apontar impacto no orçamento executivo contratado da obra.', parametros: { obra: OBRA, prazo: 'sexta-feira' }, estado: 'aguardando_aprovacao', aprovacao: { exigida: true, papelDecisor: 'Diretoria' }, propostaPor: usuario('u-eng'), criadaEm: h(27) });
  ...
  const t4: InboxThread = {
    ...
    setorCodigo: 'COMERCIAL', responsavelId: 'u-augusto', participantes: ['u-augusto'], labels: ['lead'],
    ...
    resumoIa: 'Lead pede orçamento de galpão logístico de 2.400 m² em Anápolis; sem cadastro no Radar.',
  ...
  evt({ threadId: t4.id, tipo: 'ABERTA', em: h(20), ator: sistema, detalhe: 'conversa aberta por email (EXTERNAL)' });
  evt({ threadId: t4.id, tipo: 'ROTEADA', em: h(20), ator: sistema, detalhe: 'regra ROT-05: oportunidade comercial → Comercial' });
  ...
  const t5: InboxThread = {
    ...
    setorCodigo: 'COMPRAS', responsavelId: 'u-compras', participantes: ['u-compras', 'u-obra'], codigoObra: OBRA, labels: ['suprimentos', 'campo'],
    ...
    resumoIa: 'Montador avisa que chegou lote de parafusos M16 no lugar de M20; montagem parada até a troca.',
  ...
  evt({ threadId: t5.id, tipo: 'ABERTA', em: h(2), ator: sistema, detalhe: 'conversa aberta por whatsapp (INTERNAL)' });
  evt({ threadId: t5.id, tipo: 'ROTEADA', em: h(2), ator: sistema, detalhe: 'regra ROT-04: suprimentos → Compras · prioridade Urgente' });
  evt({ threadId: t5.id, tipo: 'ACAO', em: h(1.5), ator: usuario('u-compras'), detalhe: 'tarefa criada no EIFF Control: trocar lote de parafusos (TSK exemplo)' });
  acoes.push({ id: 'ACT-00002', threadId: t5.id, tipo: 'criar_tarefa', titulo: 'Trocar lote de parafusos M16 por M20', descricao: 'Acionar o fornecedor, coletar o lote errado e entregar M20 na obra hoje.', parametros: { obra: OBRA, prioridade: 'Alta' }, estado: 'executada', aprovacao: { exigida: false }, propostaPor: usuario('u-compras'), criadaEm: h(1.5), executadaEm: h(1.5), referencia: 'TSK-exemplo' });
  ...
  const t6: InboxThread = {
    ...
    setorCodigo: 'SISTEMA', responsavelId: 'u-admin', participantes: ['u-admin'], labels: ['sistema'],
    ...
    resumoIa: 'Financeiro relata que o botão Exportar CSV da aba Alocações não responde.',
  ...
  evt({ threadId: t6.id, tipo: 'ABERTA', em: h(50), ator: sistema, detalhe: 'conversa aberta por sistema (INTERNAL)' });
  evt({ threadId: t6.id, tipo: 'ACAO', em: h(49), ator: usuario('u-admin'), detalhe: 'ação aprovada: criar job de correção' });
  evt({ threadId: t6.id, tipo: 'JOB', em: h(49), ator: sistema, detalhe: 'job JOB-00001 enviado ao provider MANUAL (a Factory ainda não recebe jobs do Inbox)' });
  acoes.push({ id: 'ACT-00003', threadId: t6.id, tipo: 'criar_job', titulo: 'Corrigir exportação CSV da aba Alocações', descricao: 'A prop csv da Tabela não está ligada na aba Alocações de Cadastros.', parametros: { tela: 'Cadastros › Alocações' }, estado: 'executada', aprovacao: { exigida: true, papelDecisor: 'Administrador', decisao: 'aprovada', decididaPor: 'u-admin', decididaEm: h(49), motivo: 'defeito reproduzível' }, propostaPor: usuario('u-admin'), criadaEm: h(49), executadaEm: h(49), jobId: 'JOB-00001' });
  jobs.push({ id: 'JOB-00001', threadId: t6.id, acaoId: 'ACT-00003', titulo: 'Corrigir exportação CSV da aba Alocações', objetivo: 'O botão Exportar CSV da aba Alocações exporta as linhas filtradas e ordenadas, como em Radar · Empresas.', contexto: ['CLAUDE.md § Bloco 5 de UX (exportação CSV)', 'src/screens/Cadastros.tsx (aba Alocações)', 'src/ui/Tabela.tsx (prop csv)'], criteriosAceite: ['Exportar CSV funciona na aba Alocações', 'Nenhuma outra tela muda de comportamento'], provider: 'MANUAL', estado: 'ENVIADO', criadoEm: h(49), criadoPor: 'u-admin' });

  // T7 — contato desconhecido, sem classificacao: triagem humana pendente
  const t7: InboxThread = {
    ...
    participantes: [], labels: [], sla: { primeiraRespostaAte: slaDe(cfg, 'Normal', h(1)) }, abertaEm: h(1), ultimaMensagemEm: h(1), ultimaInboundEm: h(1),
  };
  threads.push(t7);
  ...
  evt({ threadId: t7.id, tipo: 'ABERTA', em: h(1), ator: sistema, detalhe: 'conversa aberta por whatsapp (EXTERNAL) · contato não identificado · sem classificação: triagem humana' });
  ...
  const t8: InboxThread = {
    ...
    setorCodigo: 'FINANCEIRO', responsavelId: 'u-fin', participantes: ['u-fin'], codigoObra: OBRA, labels: ['medição'],
    ...
    resumoIa: 'Cliente envia a medição 03 assinada; faturamento pode seguir.',
  ...
  evt({ threadId: t8.id, tipo: 'ABERTA', em: h(80), ator: sistema, detalhe: 'conversa aberta por email (EXTERNAL)' });
  evt({ threadId: t8.id, tipo: 'STATUS', em: h(78), ator: usuario('u-fin'), detalhe: 'medição registrada no EIFF Control', antes: 'EM_ATENDIMENTO', depois: 'RESOLVIDA' });
  ...
  const t9: InboxThread = {
    ...
    setorCodigo: 'FORNECEDORES', participantes: [], labels: ['automático'],
    ...
    resumoIa: 'Fornecedor de tinta pergunta o endereço da fábrica; respondido com a orientação padrão.',
  ...
  evt({ threadId: t9.id, tipo: 'ABERTA', em: h(40), ator: sistema, detalhe: 'conversa aberta por whatsapp (EXTERNAL)' });
  evt({ threadId: t9.id, tipo: 'STATUS', em: h(40), ator: { tipo: 'ia', nome: 'Assistente EIFF' }, detalhe: 'nível A: respondido com orientação padrão', antes: 'TRIADA', depois: 'RESOLVIDA' });
  ...
  const t10: InboxThread = {
    ...
    setorCodigo: 'JURIDICO', participantes: [], codigoObra: OBRA, labels: ['contrato'],
    ...
    resumoIa: 'Jurídico do cliente notifica sobre reajuste contratual e pede resposta formal.',
  ...
  evt({ threadId: t10.id, tipo: 'ABERTA', em: h(6), ator: sistema, detalhe: 'conversa aberta por email (EXTERNAL)' });
  evt({ threadId: t10.id, tipo: 'ROTEADA', em: h(6), ator: sistema, detalhe: 'regra ROT-01: tema jurídico → Jurídico · nível C: humano obrigatório · setor sem responsável padrão' });

  return { setores, membros: MEMBROS_SEED, contatos, threads, mensagens, eventos, sugestoes, acoes, jobs, configuracao: cfg, origem: 'seed' };
}
