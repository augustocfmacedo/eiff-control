  ...
  return (
    <Modal title={c.nome ? c.nome : 'Novo colaborador'} onClose={onClose}>
      <div className="form">
        ...
        <Field label="Função" req hint={ds.funcoes?.length ? 'Do catálogo em Cadastros › Funções; o custo/hora padrão da função preenche o campo quando vazio' : 'Cadastre o catálogo em Cadastros › Funções'}><Input list="funcoes" value={c.funcao} onChange={(e) => { const nome = e.target.value; const f = (ds.funcoes ?? []).find((x) => x.ativa && x.nome === nome); up({ funcao: nome, ...(f?.custoHoraPadrao !== undefined && !c.custoHora ? { custoHora: f.custoHoraPadrao } : {}) }); }} /><datalist id="funcoes">{[...(ds.funcoes ?? []).filter((f) => f.ativa).map((f) => f.nome), ...FUNCOES_PADRAO.filter((f) => !(ds.funcoes ?? []).some((x) => x.nome === f))].map((f) => <option key={f} value={f} />)}</datalist></Field>
