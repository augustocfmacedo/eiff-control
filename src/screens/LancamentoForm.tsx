...
import { Field, Input, Modal, MoedaInput, NumberInput, Select, erroDoCampo, money, tentar } from '../ui/components';
...
export function LancamentoForm({ inicial, onClose, onErro, onOk }: { inicial: Lancamento; onClose: () => void; onErro: (m: string) => void; onOk: (m: string) => void }) {
  ...
  const erro = (re: RegExp) => erroDoCampo(erros, re); // validacao inline por campo (as mensagens continuam vindo do store)
  ...
  return (
    <Modal title={novo ? 'Novo lançamento' : `Lançamento ${l.id}`} onClose={onClose} wide>
      <div className="form">
        ...
        <Field label="Categoria" req erro={erro(/^Categoria/)}>
        ...
        <Field label="Código Obra" req={calc.grupoFluxo === 'Custos Diretos de Obras' || calc.grupoDre === 'Receita Operacional'} erro={erro(/Código Obra|não cadastrada/)}>
        ...
        <Field label="Contraparte" req erro={erro(/^Contraparte/)}><Input list="cps" value={l.contraparte} onChange={(e) => up({ contraparte: e.target.value })} /><datalist id="cps">{contrapartes.map((c) => <option key={c} value={c} />)}</datalist></Field>
        ...
        <Field label="Descrição" req full erro={erro(/^Descrição/)}><Input value={l.descricao} onChange={(e) => up({ descricao: e.target.value })} /></Field>
        <Field label="Competência" req hint="Regime de competência (DRE)" erro={erro(/^Competência/)}><Input type="date" value={l.competencia} onChange={(e) => up({ competencia: e.target.value })} /></Field>
        <Field label="Vencimento" req hint="Previsão de caixa e aging" erro={erro(/^Vencimento/)}><Input type="date" value={l.vencimento} onChange={(e) => up({ vencimento: e.target.value })} /></Field>
        ...
        <Field label="Probabilidade (%)" hint="Aplica-se a entradas futuras" erro={erro(/^Probabilidade/)}><NumberInput value={Math.round(l.probabilidade * 100)} onChange={(v) => up({ probabilidade: v / 100 })} min={0} max={100} /></Field>
        <Field label="Conta financeira" req erro={erro(/^Conta financeira/)}><Select value={l.contaFinanceira} onChange={(v) => up({ contaFinanceira: v })} options={ds.contas.filter((c) => c.ativa).map((c) => c.instituicao)} /></Field>
        <Field label="Valor bruto" req erro={erro(/^Valor bruto/)}><MoedaInput value={l.valorBruto} onChange={(v) => up({ valorBruto: v })} /></Field>
        <Field label="Retenções / impostos" erro={erro(/^Retenções/)}><MoedaInput value={l.retencoes} onChange={(v) => up({ retencoes: v })} /></Field>
        <Field label="Desconto"><MoedaInput value={l.desconto} onChange={(v) => up({ desconto: v })} /></Field>
        <Field label="Multa / juros"><MoedaInput value={l.multaJuros} onChange={(v) => up({ multaJuros: v })} /></Field>
      ...
      {erros.length > 0 && <div className="alert bad small" style={{ marginTop: 12 }}>{erros.length === 1 ? '1 campo a corrigir' : `${erros.length} campos a corrigir`}: {erros.join(' ')}</div>}
      {impacto && (
