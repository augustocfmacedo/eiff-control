...
- Telas em `src/screens/`; componentes em `src/ui/`. Sistema visual em `src/styles.css` (tokens: `--brand` laranja EIFF #d1481c como único acento, superfícies grafite em 3 níveis, Manrope com dígitos tabulares; tema escuro padrão, claro por `data-theme="light"` com botão no topo). Ícones vetoriais em `src/ui/icons.tsx` (`Icon`, `Marca` com as três barras da identidade). Não usar emojis como ícones nem cores fora dos tokens. Hooks do React sempre antes de qualquer `return` antecipado (o ESLint bloqueia o build se violado).
